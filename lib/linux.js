...
async function downloadUpdate(downloadUrl, logger) {
  ...
  try {
    await patchDesktopFile(updateFilePath);
  } catch (e) {
 ...
 * @returns {Promise<DesktopFile>}
 */
async function extractDesktopFile(appImagePath) {
  ...
  const file = await DesktopFile.load(path.join(root, 'appimage.desktop'));
...
async function patchDesktopFile(appImagePath) {
  const appImageDesk = await extractDesktopFile(appImagePath);
