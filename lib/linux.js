...
function downloadUpdate(downloadUrl) {
  ...
  return checkAppImageFile(appImagePath)
    ...
    .then((tempFile) => {
      return setExecFlag(tempFile);
    })
    .then((tempFile) => {
      return replaceFile(tempFile, appImagePath);
    })
    .then((appImage) => {
      return extractDesktopFileFromAppImage(appImage);
    })
    .then((desktopFileContent) => {
      return patchDesktopFile(desktopFileContent);
    })
}
...
function downloadFile(url, tempPath) {
  return new Promise((resolve, reject) => {
    request.download(url, tempPath, (err, response) => {
      if (err) {
        reject(err);
      } else {
        resolve(tempPath);
